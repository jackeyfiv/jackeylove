...
def dice_metric_binary(pred, target, smooth=1e-5):
    ...
    二类Dice评估指标
    Args:
        pred: 模型预测的类别 (B, H, W) 值为0或1
        target: 真实标签 (B, H, W) 值为0或1
        smooth: 平滑系数避免除零
    Returns:
        dice_score: 二类Dice分数
    """
    # 修正维度：求和应在最后两个维度 (H, W) 即 dim=(1, 2)
...
def train_model_binary(model, criterion, optimizer, scheduler, train_loader, num_epochs, total_epoch, model_name,
...
if __name__ == '__main__':
    model = Stage_SSM(num_class=2)  # 从3改为2
    model = model.to(device)
    # model = nn.DataParallel(model)
    dir_img = Path(r'E:\1\camus\train/img')
    dir_mask = Path(r'E:\1\camus\train\mask')
...
############################################################################  以下是训练的相关设置代码
    ...
    train_model_binary(model=model, criterion=criterion, optimizer=optimizer, scheduler=lr_scheduler, \
                train_loader=train_loader, num_epochs=20, total_epoch=20, model_name='Stage_SSM_all',
